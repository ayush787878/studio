...
export function SuccessAnimation({ onFinished }: { onFinished: () => void }) {
  ...
  useEffect(() => {
    ...
    const finishTimer = setTimeout(() => {
      onFinished();
    }, 3200); // Remove from DOM after fade out (2500ms + 700ms)
    ...
    >
      <div className="flex flex-col items-center justify-center gap-6 animate-in fade-in duration-1000">
        ...
        <div className="text-center">
            <h1 className="text-4xl font-bold font-headline text-foreground">Analysis Complete</h1>
            <p className="text-lg text-muted-foreground mt-2">facelyze.com</p>
        </div>
      </div>
    </div>
  );
