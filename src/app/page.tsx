...
const DashboardContent = () => {
    ...
    const handleDownload = () => {
        ...
        let textContent = `Facelyze Analysis Report\n`;
        textContent += `============================\n\n`;
        
        textContent += `Date: ${new Date().toLocaleString()}\n`;
        if (userProfile?.displayName) {
            textContent += `User: ${userProfile.displayName}\n`;
        }
        textContent += `\n--- OVERALL SCORE ---\n`;
        textContent += `Aesthetic Score: ${analysisResult.aestheticScore} / 100\n\n`;

        if (analysisResult.generalTraits) {
            textContent += `--- GENERAL TRAITS ---\n`;
            textContent += `Face Shape: ${analysisResult.generalTraits.faceShape}\n`;
            textContent += `Eye Color: ${analysisResult.generalTraits.eyeColor}\n`;
            textContent += `Hair Color: ${analysisResult.generalTraits.hairColor}\n`;
            textContent += `Skin Tone: ${analysisResult.generalTraits.skinTone}\n\n`;
        }

        if (analysisResult.overallImpression) {
            textContent += `--- OVERALL IMPRESSION ---\n`;
            textContent += `Rating: ${analysisResult.overallImpression.rating} / 100\n`;
            textContent += `Summary: ${analysisResult.overallImpression.text}\n\n`;
        }
        
        if (analysisResult.specificRatings) {
            textContent += `--- SPECIFIC RATINGS ---\n`;
            textContent += `Overall Harmony: ${analysisResult.specificRatings.overall} / 100\n`;
            textContent += `Aesthetic Potential: ${analysisResult.specificRatings.potential} / 100\n`;
            textContent += `Masculinity/Femininity: ${analysisResult.specificRatings.masculinity} / 100\n`;
            textContent += `Jawline: ${analysisResult.specificRatings.jawline} / 100\n`;
            textContent += `Cheekbones: ${analysisResult.specificRatings.cheekbones} / 100\n`;
            textContent += `Skin Quality: ${analysisResult.specificRatings.skinQuality} / 100\n\n`;
        }
        
        if (userProfile && analysisResult.featureAnalysis) {
            textContent += `--- DETAILED FEATURE ANALYSIS ---\n`;
            analysisResult.featureAnalysis.forEach(feature => {
                textContent += `\nFeature: ${feature.feature}\n`;
                textContent += `Rating: ${feature.rating} / 100\n`;
                textContent += `Analysis: ${feature.analysis}\n`;
            });
            textContent += `\n`;
        }
        
        if (userProfile && analysisResult.skincareRecommendations) {
            textContent += `--- SKINCARE RECOMMENDATIONS ---\n`;
            analysisResult.skincareRecommendations.forEach((rec, index) => {
                textContent += `${index + 1}. ${rec.recommendation}\n`;
                textContent += `   Reason: ${rec.reason}\n\n`;
            });
        }
        ...
        textContent += `============================\n`;
        textContent += `Report generated by Facelyze.com\n`;

        const blob = new Blob([textContent], { type: 'text/plain;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = 'facelyze_analysis.txt';
        ...
        toast({
            ...
            description: "Your analysis text file is being downloaded."
        });
    };
